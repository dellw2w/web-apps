...
function _createEmbedWorker() { return AscEmbed.initWorker(iframe); }
        ...
        (function(h){function m(){if(window.crypto&&window.crypto.getRandomValues){var a=new Uint16Array(8);window.crypto.getRandomValues(a);var b=0;function d(){return(65536+a[b++]).toString(16).substring(1)}return d()+d()+"-"+d()+"-"+d()+"-"+d()+"-"+d()+d()+d()}function c(){return Math.floor(65536*(1+Math.random())).toString(16).substring(1)}return c()+c()+"-"+c()+"-"+c()+"-"+c()+"-"+c()+c()+c()}function e(a){this.frame=a.frame;this.guid="asc.{"+m()+"}";this.isConnected=!1;this.callbacks=[];this.events=
...
e.prototype.detachEvent=function(a){this.events[a]&&(delete this.events[a],this.isConnected?this.sendMessage({type:"detachEvent",name:a}):console.log("Connector is not connected with editor"))};e.prototype._correctContextMenuItems=function(a){for(var b={guid:this.guid,items:[]},c=function(k,f){var g={id:m(),text:f.text,disabled:f.disabled};if(f.items){g.items=[];for(var l=0,n=f.items.length;l<n;l++)g.items.push(c(k,f.items[l]))}else f.onClick&&(k.contextMenuEvents[g.id]=!0,k.attachEvent(g.id,f.onClick));
return g},d=0,p=a.length;d<p;d++)b.items.push(c(this,a[d]));return b};e.prototype.addContextMenuItem=function(a){this.events.onContextMenuClick||(this.events.onContextMenuClick=function(c){this.events[c]&&this.events[c].call(this)}.bind(this));if(this.contextMenuEvents)for(var b in this.contextMenuEvents)this.contextMenuEvents.hasOwnProperty(b)&&this.detachEvent(b);this.contextMenuEvents={};this.executeMethod("AddContextMenuItem",[this._correctContextMenuItems(a)])};e.prototype.updateContextMenuItem=
function(a){this.executeMethod("UpdateContextMenuItem",[this._correctContextMenuItems(a)])};h.Asc=h.Asc?h.Asc:{};h.Asc.EditorConnector=e})(window);
